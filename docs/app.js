...
function createCard(person, editable) {
  ...
  days.forEach((day, idx) => {
    ...
    columns.forEach(col => {
      ...
      names.forEach(name => {
        ...
        if (person) {
          colDiv.appendChild(createCard(person, editable));
        }
      });
      ...
      if (editable && col === "Sick Leave") {
        colDiv.addEventListener("drop", (e) => {
          alert("❌ Only admins can edit Sick Leave.");
        });
      }
...
function moveCard(day, column, name) {
  ...
  const notInOffice = (nextWeekState[day]["On Annual Leave"] || []).length +
                      (nextWeekState[day]["Working from Home"] || []).length +
                      (nextWeekState[day]["Sick Leave"] || []).length;
